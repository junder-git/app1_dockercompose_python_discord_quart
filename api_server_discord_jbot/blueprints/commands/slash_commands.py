"""
Smart slash commands for Discord bot - Fixed version with proper choices
"""
import discord
import asyncio
...
def apply_slash_commands(bot):
    ...
    print("Applying slash commands to bot - fixed version with choices")
    
    # JAI command with proper choices and query handling
    @discord.app_commands.command(name="jai", description="Music bot control - join, leave, or play music")
    @discord.app_commands.describe(action="Choose an action or enter a song/URL to play")
    ...
    async def jai_command(interaction: discord.Interaction, action: str):
        """JAI command with fixed action parameter"""
