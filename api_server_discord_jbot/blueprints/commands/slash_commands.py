...
class SlashCommands(commands.Cog):
    ...
    @discord.app_commands.command(name="ping", description="Simple test command")
...
def apply_slash_commands(bot):
    ...
    print("Applying slash commands to bot - simplified approach")
    
    # Create the command function with full functionality
    async def jai_slash_callback(interaction: discord.Interaction, query: str = None):
        """JAI slash command callback with smart behavior"""
        print(f"JAI slash command called by {interaction.user} with query: {query}")
    ...
    # Create the command manually to control parameter behavior
    @discord.app_commands.command(name="jai", description="Join/leave voice or play music")
    @discord.app_commands.describe(query="Song name, artist, or YouTube URL (optional)")
    async def jai_command(interaction: discord.Interaction, query: str = None):
        await jai_slash_callback(interaction, query)
